...
def consumer(pipe):
    ...
    while (1):
        try:
            item = pipe.recv()
            vcu_cmd.send_motion_ctrl_msg(1, 1, item[0], item[1], 1, 0)
            time.sleep(1)
        except EOFError:
            break
...
def producer():
    ...
    # TODO: 添加摄像头标号识别：
    camera_fontIndex = camera["font_camera"]
    camera_backIndex = camera["later_camera"]

    # 摄像头部分的代码
    # 前摄像头代码
    cap = cv2.VideoCapture(camera_fontIndex)
    # 检查摄像头是否成功打开
    if not cap.isOpened():
        print("无法打开前摄像头")
        exit()
    ...
    cap.set(4, 768)
    # 后摄像头代码
    cap1 = cv2.VideoCapture(camera_backIndex)
    ...
    while True:
        ...
        if text == []:
            ...
            for texts in text:
                ...
                elif textdate == 'stop' and direct == '空挡':
                    # print("test2")
                    direct = "后退"
...
if __name__ == "__main__":
    ...
    # #     vcu_cmd.send_motion_ctrl_msg(1, 1, "空挡", i * 1.1, 0, 0)
    # # for j in range(120,-120,-5):
